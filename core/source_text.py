...
def clean_source_text(value: object | None) -> str:
    ...
    return _normalize_editorial_dashes(strip_html("" if value is None else str(value)))


def clean_plain_source_text(value: object | None) -> str:
    """Decode entities and normalize spacing while preserving literal markup."""

    return _normalize_editorial_dashes(
        normalize_text(unescape("" if value is None else str(value)))
    )
