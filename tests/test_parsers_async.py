...
async def test_global_training_fetch_yields_official_mid_career_course():
    ...
    masters = by_title["DAAD Study Scholarships for Master's Studies"]
    assert masters.type == OpportunityType.FELLOWSHIP
    assert masters.deadline == date(2026, 11, 2)
    assert "EUR 992" in masters.raw["amount_raw"]
    assert "tuition fees are not covered" in masters.raw["amount_raw"]
    assert "50026200" in masters.raw["application_url"]
    assert (
        masters.raw["i18n"]["ru"]["title"]
        == "Стипендии DAAD для обучения в магистратуре"
    )
    ...
    cotutelle = by_title["DAAD Bi-nationally Supervised Doctoral Degrees / Cotutelle"]
    assert cotutelle.type == OpportunityType.FELLOWSHIP
    assert cotutelle.deadline == date(2026, 11, 16)
    assert "EUR 1,400" in cotutelle.raw["amount_raw"]
    assert "57507783" in cotutelle.raw["application_url"]
    assert (
        cotutelle.raw["i18n"]["ru"]["title"]
        == "Гранты DAAD на совместную докторантуру Cotutelle"
    )
    ...
    artists = by_title["DAAD Study Visits for Academics – Artists and Architects"]
    assert artists.type == OpportunityType.GRANT
    assert artists.deadline == date(2026, 8, 17)
    assert "EUR 2,150" in artists.raw["amount_raw"]
    assert "50110016" in artists.raw["application_url"]
    assert (
        artists.raw["i18n"]["ru"]["title"]
        == "Стажировки DAAD для преподавателей искусства и архитектуры"
    )
