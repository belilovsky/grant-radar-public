...
`https://avds.digital`. Канонический контракт фиксирует выпуск 4.7.0,
проверенный 16 августа 2026 года. `ui.qdev.run` перенаправляет на тот же
канонический адрес.

Архитектурная запись AV DS отдельно фиксирует основу `4.0.0-rc.1`. Это не
противоречит публичному выпуску 4.7.0: первая версия обозначает базовый кандидат
