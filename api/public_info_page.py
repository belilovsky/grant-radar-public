...
@media (min-width: 1440px) {
  .info-layout { align-items: start; min-height: 0; }
  .hero { height: auto; align-content: start; }
  .cards { height: auto; grid-template-rows: none; gap: 12px; }
  .info-card { min-height: 0; align-content: start; }
}
@media (min-width: 2200px) {
  .info-layout { min-height: 0; }
  .cards { gap: 14px; }
...
PAGES: dict[str, dict[str, dict[str, object]]] = {
    "terms": {
        "ru": {
            ...
            "intro": "QAZ.FUND собирает открытые программы поддержки и ведёт к условиям организатора; заявку сервис не принимает.",
            "sections": [
                (
                    "Что делает сервис",
    ...
    "data-policy": {
        "ru": {
            ...
            "intro": "Показываем открытые сведения со ссылкой на первоисточник и отмечаем, каких данных в карточке нет.",
            "sections": [
                (
                    "Первичные источники",
...
@media(max-width:820px){{.back,.langs a,.footer a{{display:inline-flex;align-items:center;min-height:44px}}.langs a{{justify-content:center;min-width:44px}}}}
</style></head><body><main class="shell"><div class="topbar"><a class="back" href="{escape(home, quote=True)}">← {escape(back_label)}</a><nav class="langs" aria-label="Language"><a class="{'active' if active_lang == 'kk' else ''}" href="{escape(kk_href, quote=True)}" lang="kk"{' aria-current="page"' if active_lang == 'kk' else ''}>KAZ</a><a class="{'active' if active_lang == 'ru' else ''}" href="{escape(ru_href, quote=True)}" lang="ru"{' aria-current="page"' if active_lang == 'ru' else ''}>RU</a><a class="{'active' if active_lang == 'en' else ''}" href="{escape(en_href, quote=True)}" lang="en"{' aria-current="page"' if active_lang == 'en' else ''}>EN</a></nav></div><div class="info-layout"><section class="hero" data-avds-component="hero-band"><span class="eyebrow">{escape(str(page["eyebrow"]))}</span><h1>{escape(str(page["heading"]))}</h1><p>{escape(str(page["intro"]))}</p></section><section class="cards">{sections}</section></div><footer class="footer"><a class="footer-contact" href="mailto:contact@qaz.fund">contact@qaz.fund</a><span>QAZ.FUND – {escape(footer_label)}</span><span><a href="{escape(status, quote=True)}">{escape({"ru": "Статус источников", "kk": "Дереккөз мәртебесі", "en": "Source status"}[active_lang])}</a></span></footer></main></body></html>"""
